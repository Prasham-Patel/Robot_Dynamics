...
M2 = [1 0 0 0;
       0 0 1 L2/2;
       0 -1 0 L1;
       0 0 0 1]; %Pose of link {2} in frame {0}
M3 =  [1 0 0 0;
       0 -1 0 L2;
       0 0 -1 L1-L3/2;
       0 0 0 1]; %Pose of link {3} in frame {0}
M4 =  [0 1 0 0;
       0 0 -1 L2;
       -1 0 0 L1-L3;
       0 0 0 1]; %Pose of link {4} in frame {0}

M01 = M1;       % ...; Pose of link {1} in frame {0}
M12 = inv(M1) * M2;  % ...; Pose of link {2} in frame {1}
M23 = inv(M2) * M3;  % ...; Pose of link {3} in frame {2}
M34 = inv(M3) * M4;  % ...; Pose of the body frame {4} in frame {3}
...
%% Changing Spatial Inertia and Payload
...
fprintf('----------------------Dynamic Control of a 3-DoF Arm with 1kg payload--------------------\n');
...
for jj = 1 : nPts - 1
    ...
    % Use the equations of motion to calculate the necessary torques to trace
    % the trajectory
    Ftipmat = ones(N,6); % no end effector force
    taumat = InverseDynamicsTrajectory(q', qd', qdd', ...
        g, Ftipmat, Mlist, Glist, S);
...
fprintf('Simulate the robot...');
title('Inverse Dynamics Control with pay load');
robot.plot(qtt(1:10:end,:));
fprintf('Done.\n');
